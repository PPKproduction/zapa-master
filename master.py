...
class Daemon:
    ...
    def update_quota(self, uid: bytes, quota_diff: int):
        ...
        l_uid = l_uid.strip("'")
        _l.error(l_uid)
        sql = f"SELECT Remaining, Name, Surname, UID FROM UserList WHERE TAGID = '{str(l_uid)}'"
        ...
        try:
            ...
            _l.error(f'Cannot modify data into internal DB')

        branch = UpdaterSettings["branch_office_id"]
        curdate = datetime.now().date()
        curtime = datetime.now().time()
        try:
            sql = f"INSERT INTO WaterUsageExt (UID, TAGID, Date, Time, UsedWater,BranchID, ADDR) VALUES ('{result[3]}', '{str(l_uid)}', '{curdate}','{curtime}', {quota_diff},'{branch}','{src}')"
            CommitToExtDB(sql)
            sql = f"INSERT INTO WaterUsage (UID, TAGID, Date, Time, UsedWater, ADDR, SentFlag) VALUES ('{result[3]}', '{str(l_uid)}', '{curdate}','{curtime}', {quota_diff},'{src}', TRUE )"
            CommitToIntDB(sql)

        except OSError:
            _l.error(f'Cannot store data into internal DB')
        except:
            sql = f"INSERT INTO WaterUsage (UID, TAGID, Date, Time, UsedWater, ADDR, SentFlag) VALUES ('{result[3]}', '{str(l_uid)}', '{curdate}','{curtime}', {quota_diff},'{src}', FALSE )"
            CommitToIntDB(sql)
...
def CommitToExtDB(SQLstatement):
    try:
        ...
        _l.error(f'external database Error: {str(e)}')
        raise ConnectionError


def CommitToIntDB(SQLstatement):
    try:
        internlDB = mysql.connector.connect(
            user='root',
            password='zapamasterdb',
            database='ZapaDB'
        )
        intcurs = internlDB.cursor()
        intcurs.execute(SQLstatement)
        internlDB.commit()
        intcurs.close()
        internlDB.close()
        return 1
        ...
        _l.error(f'internal database Error: {str(e)}')
        raise OSError


def FetchFromIntdb(SQLstatement):
    try:
        internlDB = mysql.connector.connect(
            user='root',
            password='zapamasterdb',
